...
`anchor` is the ID of the element used as a reference, and  `anchor-to` which side of the anchor to use: "top", "bottom", "right", or "left"; with an optional postfix of "start" or "end" ("center" is the default). The demo below uses real Popovers:

<div class="w-full bg-gradient-to-r from-fuchsia-500 to-purple-600">
  {{ catalog.irender("Pop.AnchorPreview") }}
</div>
